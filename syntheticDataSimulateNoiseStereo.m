...
%%%an increasing Gaussian noise in both robot and camera motion and also stereo.
...
for noise = 1:length(NoiseCoeffT)
    ...
    for j = 1:nTimes    
        ...
        Tleft2rightNoise = Tleft2right*[rodrigues(NoiseCoeffR(noise)*randn(3, 1)), NoiseCoeffT(noise)*randn(3, 1);0 0 0 1];
...
xlabel('noise coeff (mm)'), ylabel('error (mm)'),
title('translational error (right)')
legend('ATA', 'Barreto', 'TSAI', 'DQ'), grid on, axis tight
hold off
figure(4), 
...
xlabel('noise coeff (degree)'), ylabel('error (degree)')
title('rotational error (right)')
legend('ATA', 'Barreto', 'TSAI', 'DQ'), grid on, axis tight
hold off

%Save results for plotting-gui
% save resultABNoiseStereo.mat